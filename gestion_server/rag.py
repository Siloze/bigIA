    ...
    for filename in os.listdir(path):
        if filename.endswith(".txt"):
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                full_text = f.read()
                data.append({"name": filename, "content": full_text}) 
    return data

class RAG:
    
    def __init__(self, data: List[RAG_DATA], modele_path: str = None, embedder: SentenceTransformer = None):
        self.data: List[RAG_DATA] = data
    ...
    def init_chunks(self):
        ...
        for doc in self.data:
            name = doc["name"]
            content = doc["content"]
            chunks.extend(split_text_recursive(content))
        self.chunks = chunks
